...
import { modules } from '@/components/module/utils';
import './surveys.css';
...
const SurveyTable = ({ selectedLevel }: { selectedLevel: string }) => {
    ...
    return (
        <div>
            ...
            <table className="table-hover table-striped mb-5 table">
                ...
                <tbody>
                    {levelData.map((row) => (
                        <tr key={row}>
                            <td>{row}</td>
                            {questionHeaders[currentQuestion].map(
                                (header, index) => (
                                    <td key={header}>
                                        <input
                                            type="radio"
            ...
            <div className="button-container">
                ...
                {isLastQuestion && (
                    <input type="email" placeholder="Key in your email here" />
                )}
                {isLastQuestion ? (
